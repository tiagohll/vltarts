                ...
                <Header />
                    <div className='flex flex-col items-center'>
                        <div className='container flex flex-col lg:flex-auto'>
                            ...
                            <section className='flex flex-col items-center text-center m-5 gap-4'>
                                {!isLoading ? (
                                   page == "myorders" ? (
                                        userInfo?.orders ? (
                                            <>
                                                <div className='w-2/4 p-2 rounded'>
                                                    <Table>
                                                        ...
                                                        {Object.entries(userInfo?.orders || {}).map(([itemName, itemData]) => {
                                                            return (
                                                                <TableRow key={itemName}>
                                                                    ...
                                                                    <TableCell className={`${itemData.stats == "Entregue" && ('text-green-600') || itemData.stats == "Cancelado" && ('text-red-600') || itemData.stats == "Em andamento" && ('text-yellow-500') } `}>
                                                                        {itemData.stats}
                                                                    </TableCell>
                                                ...
                                                <span className='font-medium'>Você ainda não fez nenhum pedido. <Link className='font-bold' to='/products'>Ver produtos.</Link></span>
                                            )
                                    ) : (
                                        <div className='w-2/4 p-2 rounded flex flex-col bg-zinc-300'>

                                            <h2 className='font-medium'>Nome: <span className='font-bold'>{userInfo?.name}</span></h2>
                                            <h2 className='font-medium'>E-mail: <span className='font-bold'>{userInfo?.email}</span></h2>
                                            <h2 className='font-medium'>Telefone: <span className='font-bold'>{userInfo?.phone}</span></h2>
                                        </div>
                                    ) 
                                ) : (
                    ...
                    {signOutModal && (
                        <div className={`${animateBackground} flex items-center justify-center modal top-0 w-full h-screen bg-white bg-opacity-50 absolute`}>
                            <div className={`${animateModal} modal bg-zinc-200 w-1/4 shadow p-2 py-10 text-center flex flex-col items-center font-bold gap-2 text-lg`}>
