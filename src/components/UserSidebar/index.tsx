...
export default function UserSidebar({ pageA, setPage }: SettingsSideBarProps) {
  ...
  return (
    <>
      <div className="hidden sm:block xl:w-1/12 fixed p-1 py-2 bg-zinc-300 gap-2 rounded m-5">
        {userPages.map((page) => (
          <button
            key={page.id}
            className={`flex items-center w-full p-2 my-1 rounded gap-2 ${
              pageA == page.path && "bg-zinc-400 bg-opacity-50 font-bold"
            } duration-300 hover:bg-zinc-400 hover:bg-opacity-50 font-medium`}
            onClick={() => {
              setPage(page.path);
            }}
          >
      ...
      {signOutModal && (
        <div
          className={`${animateBackground} flex items-center justify-center modal left-0 top-0 w-full h-screen bg-white bg-opacity-50 absolute px-2`}
        >
          <div
            className={`${animateModal} bg-zinc-200 modal min-w-fit shadow p-2 py-10 text-center flex flex-col items-center font-bold gap-2 text-lg`}
          >
            <h1 className="flex gap-2 text-center justify-center">
              <AlertTriangle className="text-red-600 hidden sm:block" /> Você
      ...
      <div className="sm:hidden px-5 py-2 w-full flex overflow-x-scroll">
        {userPages.map((page) => (
          <button
            key={page.id}
            className={`${
              pageA == page.path && "border-green-600"
            } border-b-2 border-transparent p-2`}
            onClick={() => {
              setPage(page.path);
            }}
          >
